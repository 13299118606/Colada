...
void qCRSWidget::onModelSelectionChanged(
	...
	const QItemSelection& deselected)
{
	...
	QString name = d->proxy->data(
		d->proxy->index(index.row(), 0)).toString();
	QString authName = d->proxy->data(
		d->proxy->index(index.row(), 1)).toString();
	QString code = d->proxy->data(
		d->proxy->index(index.row(), 2)).toString();

	OGRSpatialReference sr;
	sr.SetFromUserInput((authName + ":" + code).toUtf8());
	sr.SetLinearUnitsAndUpdateParameters(SRS_UL_METER, 1);
