...
qScienceSpinBoxPrivate::qScienceSpinBoxPrivate(qScienceSpinBox &q)
    : q_ptr(&q) {}

qScienceSpinBoxPrivate::~qScienceSpinBoxPrivate() {}

void qScienceSpinBoxPrivate::init(QWidget *parent) {
  Q_Q(qScienceSpinBox);
  initLocalValues(parent);
  q->setDecimals(8);
  q->QDoubleSpinBox::setDecimals(1000);

  // set Range to maximum possible values
  constexpr double doubleMax = std::numeric_limits<double>::max();
  q->setRange(-doubleMax, doubleMax);

  v = new QDoubleValidator(q);
  v->setDecimals(1000); // (standard anyway)
  v->setNotation(QDoubleValidator::ScientificNotation);
  q->lineEdit()->setValidator(v);
}

void qScienceSpinBoxPrivate::initLocalValues(QWidget *parent) {
...
qScienceSpinBox::qScienceSpinBox(QWidget *parent)
    : QDoubleSpinBox(parent), d_ptr(new qScienceSpinBoxPrivate(*this)) {
  Q_D(qScienceSpinBox);
  d->init(parent);
}

qScienceSpinBox::qScienceSpinBox(qScienceSpinBoxPrivate* pimpl, QWidget *parent)
    : QDoubleSpinBox(parent), d_ptr(pimpl) {
...
int qScienceSpinBox::decimals() const { 
  Q_D(const qScienceSpinBox);
  return d->dispDecimals; 
}

void qScienceSpinBox::setDecimals(int value) { 
  Q_D(qScienceSpinBox);
  d->dispDecimals = value;
}
...
QString qScienceSpinBox::textFromValue(double value) const {
  ...
  QString str = locale().toString(value, 'e', d->dispDecimals);
...
double qScienceSpinBox::round(double value) const {
  Q_D(const qScienceSpinBox);
  const QString strDbl = locale().toString(value, 'g', d->dispDecimals);
...
// QDoubleSpinBoxPrivate::isIntermediateValue
bool qScienceSpinBox::isIntermediateValue(const QString &str) const {
   ...
   */
  const int dotindex = str.indexOf(d->delimiter);
...
QVariant qScienceSpinBox::validateAndInterpret(QString &input, int &pos,
  ...
  if (cachedText == input && !input.isEmpty()) {
    state = cachedState;
    QSBDEBUG() << "cachedText was"
               << "'" << cachedText << "'"
               << "state was " << state << " and value was " << cachedValue;
    return cachedValue;
  }
  ...
  case 1:
    // if only char is '+' or '-'
    if (copy.at(0) == d->delimiter || (plus && copy.at(0) == QLatin1Char('+')) ||
  ...
  // Test possible 'Invalid' reasons
    ...
    const int dec = copy.indexOf(d->delimiter); // position of delimiter
    // if decimal separator (delimiter) exists
    if (dec != -1) {
      // not two delimiters after one other (meaning something like ',,')
      if (dec + 1 < copy.size() && copy.at(dec + 1) == d->delimiter &&
          pos == dec + 1) {
        copy.remove(dec + 1,
      ...
      // two space chars is invalid
               ...
               (!d->thousand.isSpace() || secondLast.isSpace())) {
        state = QValidator::Invalid;
        QSBDEBUG() << __FILE__ << __LINE__ << "state is set to Invalid";
        goto end;
      }
  ...
  // block of remaining test before 'end' mark
  {
    ...
    if (!ok) {
      ...
      if (d->thousand.isPrint()) {
        ...
        if (max < 1000 && min > -1000 && copy.contains(d->thousand)) {
          state = QValidator::Invalid;
          QSBDEBUG() << __FILE__ << __LINE__ << "state is set to Invalid";
          goto end;
        }
        ...
        num = loc.toDouble(copy, &ok);
        QSBDEBUG() << d->thousand << num << copy << ok;
