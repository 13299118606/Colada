...
qColadaNewProject::Param qColadaNewProject::getParam() {
  ...
  p.units = d->crsWidget->getUnits();
  p.crsName = d->crsWidget->getProxyModel()
                  ->data(d->crsWidget->getProxyModel()->index(row, 0))
                  .toString();
  p.crsAuthName = d->crsWidget->getProxyModel()
                      ->data(d->crsWidget->getProxyModel()->index(row, 1))
                      .toString();
  p.crsCode = d->crsWidget->getProxyModel()
                  ->data(d->crsWidget->getProxyModel()->index(row, 2))
                  .toString();
...
void qColadaNewProject::onCrsCheckBoxStateChanged(int state) {
  ...
  if (state == Qt::Unchecked) {
    d->crsWidget->getUnitsLineEdit()->setEnabled(true);
    ...
    d->crsWidget->getUnitsLineEdit()->setDisabled(true);
...
void qColadaNewProject::onButtonBoxClicked(QAbstractButton* button) {
  ...
  if (button == d->buttonBox->button(QDialogButtonBox::Ok)) {
    ...
    if (!dbcore::createProjectFolders(qDir.absolutePath())) {
      QMessageBox msgBox;
      msgBox.setIcon(QMessageBox::Critical);
    ...
    if (!dbcore::fillInfoTable(p.owner, p.prjName, p.prjDir, p.units,
                               p.crsAuthName, p.crsCode, p.crsName)) {
      QMessageBox msgBox;
      msgBox.setIcon(QMessageBox::Critical);
