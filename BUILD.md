Build and Package Colada
==============================

This document summarizes how to build and package Colada on Windows. Instructions for Linux and macOS are similar.
...
* Setting up your git account:
    ...
    * If not already done, email `FirstName LastName <firstname.lastname@TierraColada.com>` to be granted access to
    the [TierraColada/Colada](https://github.com/TierraColada/Colada) repository.
...
mkdir W
cd /c/W
git clone https://github.com/TierraColada/Colada.git Colada
```
...
1. Start [CMake GUI](https://cmake.org/runningcmake/), select source directory `C:\W\Colada` and set build directory to `C:\W\Colada-rel`.
2. Add an entry `Qt5_DIR` pointing to `C:/Qt/${QT_VERSION}/${COMPILER}/lib/cmake/Qt5`.
2. Generate the project.
3. Open `C:\W\Colada-rel\Colada.sln`, select `Release` and build the project.

<b>Option 2: Command Line</b>

1. Start the [Command Line Prompt](http://windows.microsoft.com/en-us/windows/command-prompt-faq)
2. Configure and build the project in `C:\W\Colada-rel` by typing the following commands:

```bat
cd C:\W\
mkdir Colada-rel
cd Colada-rel
cmake -G "Visual Studio 14 2015 Win64" -DQt5_DIR:PATH=`C:/Qt/${QT_VERSION}/${COMPILER}/lib/cmake/Qt5 ..\Colada
...
1. In the `C:\W\Colada-rel\Slicer-build` directory, open `Slicer.sln` and build the `PACKAGE` target
...
```bat
cd C:\W\Colada-rel\Slicer-build
